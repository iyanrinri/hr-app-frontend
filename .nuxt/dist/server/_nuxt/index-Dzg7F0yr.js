...
import { u as useEmployeeOvertimeHistory, a as useEmployeeTotalHours } from "./useOvertime-BEYm8O2P.js";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!selectedEmployeeId.value) {
        ...
        _push(`</div><div class="text-4xl font-bold">${ssrInterpolate(unref(isLoadingTotal) ? "..." : unref(totalHoursData)?.totalHours.toFixed(1))} hrs </div><p class="text-white/60 text-sm mt-2">${ssrInterpolate(unref(totalHoursData)?.totalMinutes)} minutes Total</p></div><div class="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-xl p-6 text-white shadow-lg"><div class="flex items-center justify-between mb-4"><h3 class="text-white/80 font-medium">Total Compensation</h3>`);
        _push(ssrRenderComponent(unref(DollarSign), { class: "w-8 h-8 text-white/40" }, null, _parent));
        _push(`</div><div class="text-4xl font-bold">${ssrInterpolate(unref(isLoadingTotal) ? "..." : unref(totalHoursData)?.totalAmount ? formatCurrency(unref(totalHoursData).totalAmount) : "Rp 0")}</div><p class="text-white/60 text-sm mt-2">Estimated Amount</p></div>`);
        _push(ssrRenderComponent(_component_UiCard, { class: "p-6" }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<h3 class="text-gray-500 font-medium mb-4"${_scopeId}>Request Status Breakdown</h3><div class="space-y-3"${_scopeId}><div class="flex justify-between items-center text-sm"${_scopeId}><span class="flex items-center gap-2"${_scopeId}><div class="w-3 h-3 rounded-full bg-green-500"${_scopeId}></div> Approved</span><span class="font-medium text-gray-900"${_scopeId}>${ssrInterpolate(unref(totalHoursData)?.requests?.filter((r) => r.status === "APPROVED").length || 0)}</span></div><div class="flex justify-between items-center text-sm"${_scopeId}><span class="flex items-center gap-2"${_scopeId}><div class="w-3 h-3 rounded-full bg-yellow-400"${_scopeId}></div> Pending</span><span class="font-medium text-gray-900"${_scopeId}>${ssrInterpolate(unref(totalHoursData)?.requests?.filter((r) => r.status === "PENDING").length || 0)}</span></div></div>`);
            } else {
              return [
                ...
                createVNode("div", { class: "space-y-3" }, [
                  createVNode("div", { class: "flex justify-between items-center text-sm" }, [
                    ...
                    createVNode("span", { class: "font-medium text-gray-900" }, toDisplayString(unref(totalHoursData)?.requests?.filter((r) => r.status === "APPROVED").length || 0), 1)
                  ]),
                  createVNode("div", { class: "flex justify-between items-center text-sm" }, [
                    createVNode("span", { class: "flex items-center gap-2" }, [
                    ...
                    createVNode("span", { class: "font-medium text-gray-900" }, toDisplayString(unref(totalHoursData)?.requests?.filter((r) => r.status === "PENDING").length || 0), 1)
                  ])
                ])
              ];
            }
              ...
              _push2(ssrRenderComponent(_component_UiCardContent, { class: "p-6" }, {
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    ...
                    ssrRenderList(unref(totalHoursData)?.requests, (req) => {
                      ...
                      createVNode("div", { class: "overflow-x-auto" }, [
                        createVNode("table", { class: "min-w-full text-sm" }, [
                          ...
                          createVNode("tbody", { class: "divide-y divide-gray-50" }, [
                            (openBlock(true), createBlock(Fragment, null, renderList(unref(totalHoursData)?.requests, (req) => {
                              return openBlock(), createBlock("tr", {
                                key: req.id
                              }, [
...
//# sourceMappingURL=index-Dzg7F0yr.js.map
