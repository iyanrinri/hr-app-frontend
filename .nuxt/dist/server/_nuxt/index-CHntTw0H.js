...
import { defineComponent, ref, withCtx, unref, createVNode, createTextVNode, toDisplayString, useSSRContext, computed, mergeProps } from "vue";
...
import { u as useRoute, E as Eye, w as SquarePen, x as RotateCcw, d as Trash2, X, P as Plus, y as Search, z as LayoutGrid, G as List, U as User, M as Mail, H as Briefcase } from "../server.mjs";
import "/Users/iyan/NodeStudio/hr-app-frontend/node_modules/hookable/dist/index.mjs";
import { a as useEmployees, b as useDeleteEmployee, c as useRestoreEmployee } from "./useEmployees-CEaIsP48.js";
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.employees, (employee) => {
        ...
        _push(`</div></td><td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${ssrInterpolate(employee.user?.email)}</td><td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${ssrInterpolate(employee.position)}</td><td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${ssrInterpolate(employee.department)}</td><td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${ssrInterpolate(formatDateShort(employee.joinDate))}</td><td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium"><div class="flex justify-end space-x-2">`);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const isEmployeeActive = (emp) => !emp.user?.deletedAt;
      ...
      if (unref(loading)) {
        ...
        if (viewMode.value === "grid") {
          ...
          ssrRenderList(employees.value, (emp) => {
            ...
            _push(`<p class="text-brand-cyan font-medium text-sm mt-1 truncate">${ssrInterpolate(emp.position || emp.user?.role)}</p></div><div class="space-y-3 mb-6 pt-4 border-t border-gray-50"><div class="flex items-center text-sm text-gray-500">`);
            _push(ssrRenderComponent(unref(Mail), { class: "w-4 h-4 mr-3 text-gray-400 shrink-0" }, null, _parent));
            _push(`<span class="truncate"${ssrRenderAttr("title", emp.user?.email)}>${ssrInterpolate(emp.user?.email)}</span></div><div class="flex items-center text-sm text-gray-500">`);
...
//# sourceMappingURL=index-CHntTw0H.js.map
