    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.loading) {
        ...
        ssrRenderList(__props.payslips, (payslip) => {
          _push(`<tr class="hover:bg-gray-50 transition-colors"><td class="px-6 py-4 whitespace-nowrap"><div class="text-sm font-medium text-gray-900">${ssrInterpolate(payslip.payroll?.periodStart ? formatDate(payslip.payroll.periodStart) : "N/A")}</div><div class="text-xs text-gray-500"> Gen: ${ssrInterpolate(formatShortDate(payslip.generatedAt))}</div></td><td class="px-6 py-4 whitespace-nowrap">`);
          if (payslip.payroll?.employee) {
...
//# sourceMappingURL=PayslipHistoryTable-2hg4T3Ds.js.map
