...
import { usePendingOvertimeRequests, useApproveOvertimeRequest, useRejectOvertimeRequest } from '@/hooks/useOvertime';
...
export default function PendingOvertimePage() {
  const { data: requests, isLoading } = usePendingOvertimeRequests(); // This returns array directly as per hook
  const { mutate: approveRequest } = useApproveOvertimeRequest();
  const { mutate: rejectRequest } = useRejectOvertimeRequest();

  const handleApprove = (id: string) => {
    // Optional: Add prompt for comments
  ...
  const handleReject = (id: string) => {
    ...
    if (reason) {
      rejectRequest({ id, comments: reason });
    } else if (reason === '') {
